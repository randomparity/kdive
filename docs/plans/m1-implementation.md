...
- **Scope:**
  ...
  - `accounting.estimate({selector, window})` → `{estimate_kcu, rate_kcu_per_hr, breakdown}`; `viewer` role.
- **Acceptance:** `rate` matches the size-weighted formula for sample sizes; a `cost_class` with no coefficient row raises `configuration_error`; `accounting.estimate` returns the rate×window product with a vcpu/memory breakdown; `viewer` may call it.
...
- **Scope:**
  - Pin each tool to its **lowest** sufficient role (table in [0037](../adr/0037-rbac-hardening-role-separation.md)): `accounting.set_budget`/`set_quota` → `admin`; the destructive gate's role factor → `admin` (force_crash, power off/cycle/reset, teardown); read/usage → `viewer`; lifecycle → `operator`.
  - Mock-OIDC fixture mints distinct `viewer`/`operator`/`admin` tokens per test project.
  - Negative tests: each privileged tool refuses the next-lower role (`AuthorizationError` → mapped `authorization_error`).
- **Acceptance:** `operator` is refused `set_budget`/`set_quota` and `force_crash` (three+ negative tests); `admin` succeeds; `viewer` may read usage but not request an allocation; the separated-principal fixture is importable by other test modules.
