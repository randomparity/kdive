...
- **Write (after validation):**
  ...
  2. `store.put_artifact(ArtifactWriteRequest(tenant="system", owner_kind="build-configs",
     owner_id=name, name=f"{name}.config", data=data, sensitivity=Sensitivity.REDACTED,
     retention_class="build-config"))` — the same reserved key the seed uses; a re-set
     overwrites in place.
  3. Upsert the catalog row with `source='operator'` (one transaction with the audit write,
     so the row and its `platform_audit_log` entry commit together).
...
- Read the row's `(sha256, source)` for `name`.
- If the row exists and `source='operator'` → **skip** (return 0); the operator owns it.
- Else if the row exists, `source='seed'`, and its sha256 == packaged sha256 → skip (return 0;
  unchanged fast-path).
- Else publish the packaged bytes and upsert with `source='seed'`.

So a fresh install seeds the default, a shipped packaged-fragment change still flows to
seed-owned rows on the next `migrate`, and an operator override is untouched.
...
- A shared async `upsert_build_config(conn, name, object_key, sha256, description, source)`
  the tool and seed both call (the `ON CONFLICT (name) DO UPDATE` now also sets `source`).
