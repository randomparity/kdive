...
3. **The instructions omit the recipe.** `_GATEWAY_ON_SURFACE` says only "pass a short
   description of what you want to do": no `limit`, no `detail`, no `namespace`, no exact-name
   form.

Cost measured in the #2305 evaluation: one or two extra round trips per first-time tool call,
...
- **Instructions that name the parameters** (`src/kdive/mcp/schema/tool_index.py`).
  `_GATEWAY_ON_SURFACE` gains two sentences naming `names=[...]`, `limit`, `detail`, and
  `namespace`. The `tools.search` wrapper docstring and every `Field(description=...)` stay in
  sync with the behaviour, since FastMCP serialises only those into the agent-visible schema
  (AGENTS.md, "The wrapper docstring is the agent-facing contract"). `docs/guide/reference/tools.md`
  is regenerated by `just docs`.
...
7. ADR-0472 §2 does not regress: a query that is exactly a tool name still ranks that tool first,
   ...
   `names`.
8. The committed `docs/guide/reference/tools.md` matches a fresh generation (`just docs-check`).
...
| Generated tool reference matches the registry | `focused-test` | `just docs-check` (CI-gated; run after `just docs`) |
