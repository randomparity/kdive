...
active rows with migration 0155's exact slot-derived incarnation prefix. The coordinator skips
structurally valid rows bound to another host, validates every applicable local row before the
first release, and derives each local row's outcome from its stored boot and invocation identity
against the current systemd observation. Only after every applicable local row releases does it
clear root-owned slot files and reset a retained failed unit identity.

Recovery uses this row-derived classification for valid and unreadable state alike so a valid
retained generation cannot be retired before an older active sibling is found unclassifiable.
Prepared state holds no fence and remains a file-only discard. Already terminated state remains
ordinary cleanup. Start, status, and stop retain their existing evidence path and behavior.
...
- `IncarnationAuthority.recoverable(unit)` returns the bounded prefix-match active row set;
...
- Recovery classifies every applicable local row before its first release; a classification refusal
...
- Recovery skips a structurally valid foreign-host row and cannot release an incoherent local row.
...
- Releases remain the existing one-row transactions. If a later release fails, an already released
  prefix stays committed, slot files stay present, and retry reclassifies the active remainder.
...
skips valid foreign-host rows, refuses incoherent local rows and populated or unknown cgroups,
classifies the complete applicable local set before writes, and calls the unchanged exact-binding
...
1. Tests named for cases 1-4 prove a single `recover` call clears the four fixed slot files and
   releases the complete applicable local active fence-row set for a proven-dead slot.
...
4. Tests prove every published outcome is derived from the current observation and stored identity,
   never synthesized from another invocation's result fields, and valid state cannot retire before
   an older active sibling is classified.
