...
Host of record (freeze 3): an emulated ppc64le guest (QEMU `pseries`,
`accel=tcg`, Fedora 44) on merged `main`.

Freeze 4 amended the no-production-code exclusion after criteria 1–3 proved
...
KVM, not the System's `accel`, reusing ADR-0341's multiplier, plus
...
pre-authorized; a phase still over budget is a recorded timeout. The spine token then expired
at the issuer's 3600 s mid-boot, so `mint_role_token` sets `exp` (freeze 5).

Build `KDIVE_GUEST_IMAGE_PPC64LE` via `python -m kdive build-fs` so #2381's
`fadump-capture.service` comes from customize (ADR-0345), and `KDIVE_PPC64LE_BUNDLE`
...
revision; run the drivers by node id.

Write a dated record under `docs/design/`, redacted to the charter's tokens, stating
every deviation, linked from `platform-support.md` and `live-testing.md`'s fadump-outcome
...
| 5b | task-test-not-applicable | Host identifiers in prose: no tool models host identity, so nothing could fail; read against the charter's tokens. |
