...
prepared-volume contracts owned by #2167 and #2168. Those two dependencies must land before this
...
- Streams are aborted in all exits. Console bytes are redacted before bounding and only validated,
  stable protocol `key=value` lines may leave the supervisor. Durable scratch results, not console
  text, decide success.
...
progress beyond 30 seconds under the 300-second cap, and yielded would-block polling. Existing
tests retain domain-shape, redaction, stream-release, durable-result, timeout, and teardown proofs.
The provider test file, lint, whole-tree type checking, and the full `just ci` gate must pass once
#2167 and #2168 are present on the branch.
