...
#2168 prepared volumes, durable module documents, and secret redaction.
...
- Do not duplicate #2167 attachment or #2168 volume contracts.
...
`normalized_appliance_devices`, and `validate_appliance_xml`, and #2168's `PreparedVolume`.
...
3. Re-raise `CategorizedError` before the unclassified identity exception handler.
4. Re-arm the local idle deadline after each non-empty byte chunk, capped by the immutable outer
   deadline.
5. Yield for `min(0.01, remaining idle time)` after `-2` without re-arming.
6. Run the focused test file and expect all tests to pass.
7. Run `just lint` and `just type`, then commit the implementation separately from this design.
