...
- Mode: focused-test. Contract: `probe_stack_skew()` warns that an absent portable lifecycle witness is not deployed without a strict skip, and grades a reachable witness normally. Test: `tests/integration/live_stack/test_skew.py`; expected red: generic no-build unknown text, a strict skip, or a hidden explanation. Green: `uv run python -m pytest tests/integration/live_stack/test_skew.py -q` exits 0.
