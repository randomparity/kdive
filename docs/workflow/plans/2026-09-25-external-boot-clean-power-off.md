...
`stop_and_require_inactive(mode=...)`: preparation uses `clean` (60 s on KVM, 120 s otherwise);
recovery uses `clean-on-kvm` in `target-defined` and `destroy` before it (operator option B).
...
| `src/kdive/providers/local_libvirt/lifecycle/boot/session.py` | `_Domain` gains `state`/`shutdown`; session `kvm`, `sleep`, `clock`; `StopMode`, `stop_and_require_inactive(*, mode)`; `restore_power()`; teardown comment |
| `src/kdive/providers/local_libvirt/lifecycle/boot/external_boot.py` | pass the stop mode; `restore_power()` |
...
| `tests/providers/local_libvirt/test_external_boot.py` | doubles take `mode`; recovery/preparation mode tests |
...
## Task 2 — session stop modes

Interfaces: consumes Task 1's `power_off` and `clean_shutdown_bound_s`. Provides
`StopMode = Literal["clean", "clean-on-kvm", "destroy"]` in `session.py`,
`LocalExternalBootSession.stop_and_require_inactive(self, *, mode: StopMode) -> None` and
...
- clean on KVM — `test_clean_stop_requests_shutdown`: `type="kvm"`, active, `mode="clean"`;
  events contain `domain.shutdown`, not `domain.destroy`. Red: `TypeError` on `mode=`.
- bounds — `test_clean_stop_bound`, parametrized `(domain type, mode, sleeps)`:
  `("kvm", "clean", 60)`, `(None, "clean", 120)`, `("kvm", "clean-on-kvm", 60)`;
  `honours_shutdown=False`, a fake clock whose `sleep` advances it; `len(clock.sleeps)` equals
  the expected count and the last event is `domain.destroy`.
- unaccelerated — `test_clean_on_kvm_destroys_unaccelerated`: no `type`, `mode="clean-on-kvm"`;
  `domain.destroy`, no `domain.shutdown`; caplog WARNING `destroy-unaccelerated`.
- unready — `test_destroy_mode_destroys_at_once`: KVM, `mode="destroy"`; `domain.destroy`, no
...
- `restore_power()` — five existing call sites change: `restore_power("running")` at the
  ...
  `stop_and_require_inactive(mode="destroy")`. Add a check that a second `restore_power()` on the
  active domain adds no `domain.create`.
...
3. `session.py`: add `import logging`, `import time`, `_log = logging.getLogger(__name__)`, and
   ...
   `kvm=inactive_root.get("type") == "kvm"`, `sleep`, `clock` into `_ConcreteSession`. Add
   `_UNACCELERATED_STOP_BOUND_S = 120.0` (ADR-0681). Replace `stop_and_require_inactive`:
...
Interfaces: consumes Task 2's `stop_and_require_inactive(*, mode)` and `restore_power()`.
...
- preparation — the preparation double records `mode`; a preparation test asserts `["clean"]`.
...
- recovery — through the restart double with the target running, `recover_modules` records
  `"clean-on-kvm"` for phase `target-defined` and `"destroy"` for `module-restored`.
...
Steps:
1. Doubles: both `stop_and_require_inactive` doubles take `*, mode: str` and append it to a
...
3. `external_boot.py`: `prepare` calls `stop_and_require_inactive(mode="clean")`;
   `_stop_for_recovery` calls it with `"clean-on-kvm" if metadata.phase == "target-defined" else
   "destroy"` and a one-line comment citing ADR-0681; `_abort_preparation` calls
   `restore_power()`.
