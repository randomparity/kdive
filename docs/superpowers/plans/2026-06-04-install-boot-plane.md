...
- [ ] **Step 1 (tests first):** Write `tests/providers/local_libvirt/test_install.py`. Inject fakes for every slow/host op (`connect` → a fake `_LibvirtConn` recording `defineXML`/`lookupByName`/`create`/`destroy`; `fetch_kernel`/`fetch_initrd` → write canned bytes to the given path; `readiness` → a canned structured result; an injected `boot_window`/`clock` so no real wait):
  ...
  - **kdump prerequisite (inside `install`):** with the readiness/kdump seam reporting the capture path **present**, `install` redefines; with it reporting **absent**, `install` raises `CategorizedError(CONFIGURATION_ERROR)` **before** `defineXML` (assert no redefine);
