...
> **Design refinement (TDD-surfaced): enforce-when-advertised, not fail-closed-when-absent.** `validate_disk_against_resource` reads `disk_ceiling()` and skips the bound when the resource advertises none. local-libvirt always advertises (live-derived, Task 3), so a local request is always bounded; remote-libvirt (disk-image) and fault-inject size no host disk and advertise none, so they are unbounded here — no inventory-model or `systems.toml` change, and no fixture sweep is required for them. Adding `disk_gb` (e.g. `500`) to a few local admission-test seeds is optional coverage exercising the enforce path. See spec Part 3 and ADR-0312's rejected "fail-closed-when-absent" alternative for why disk differs from the required vcpus/memory ceilings.
