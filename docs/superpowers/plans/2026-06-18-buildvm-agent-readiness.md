...
**Steps (TDD):**
1. Failing test: inject a thin `session_factory` (a `@contextmanager` taking the `_SessionFactory` signature) whose body calls the **real** `wait_for_agent_responsive` with a code-86 agent fake, a no-op sleep, a `_ticker` clock, and a tiny `timeout_s` so the gate reaches its deadline and raises the marked `CONFIGURATION_ERROR` — no `EphemeralBuildVm`/libvirt host needed. Assert `_blocking_probe` / `buildhost_agent_probe` classifies the host as `AGENT_UNREACHABLE` (FAIL), not `HOST_UNREACHABLE`. This exercises the real raise-site → read-site contract through the shared constants. Add/confirm a test that an *unmarked* `CONFIGURATION_ERROR` (e.g. no base image) stays `HOST_UNREACHABLE`. (`test_readiness.py` separately pins that the real gate populates the marker, so the two tests jointly cover the end-to-end contract.)
