...
`introspect_from_vmcore(pool, ctx, run_id, introspector)` (spec §Components): resolve Run + `debuginfo_ref` + System + recorded `build_id`, load the System's raw `vmcore` key; call the port; map `CategorizedError` → typed `ToolResponse.failure` (never a 500); JSON-serialize the already-redacted report into `data["report"]`, surface `data["truncated"]`; `suggested_next_actions=["introspect.from_vmcore", "artifacts.list"]`. Resolution failures (null `debuginfo_ref`, no `build` step, no captured core, bad uuid) → `configuration_error`.
