...
**Spec:** `docs/design/local-git-build-lane.md` · **ADR:** `docs/adr/0157-local-git-build-lane.md`
...
- ADR number is **0157**; already created. Do not pick a new one.
...
# Closed ambient escape hatches so the allowlist bounds the actual connection (ADR-0157).
...
def clone_tree(
    ...
    *, run_id: UUID, secret_registry: SecretRegistry,
    ...
    """Clone `source.remote` at `source.ref` into `workspace` (ADR-0157), allowlist-gated."""
