...
**Spec:** `docs/design/local-git-build-lane.md` · **ADR:** `docs/adr/0159-local-git-build-lane.md`
...
- ADR number is **0159**; already created. Do not pick a new one.
...
# Closed ambient escape hatches so the allowlist bounds the actual connection (ADR-0159).
...
def clone_tree(
    ...
    *, run_id: UUID, secret_registry: SecretRegistry,
    ...
    """Clone `source.remote` at `source.ref` into `workspace` (ADR-0159), allowlist-gated."""
...
**CI gates this touches (all run individually in CI):** `config-docs-check`, `resources-docs-check`, `env-docs-check` (satisfied by the registry entry from Task 1), `adr-status-check` (ADR-0159 is Accepted + the README row matches).
