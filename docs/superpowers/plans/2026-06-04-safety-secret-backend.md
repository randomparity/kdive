...
- [ ] **Step 1: Write the failing tests** (`tests/security/test_secrets.py`, using `tmp_path` + a **local** `SecretRegistry`):
  ...
  - **Acceptance — register-before-return (no-skip):** a `SecretRegistry` subclass/double whose `register` records the call; after `resolve` returns, the recorded list already contains the value. Plus the positive end-to-end: `Redactor(registry.snapshot())` built right after `resolve` masks the value.
...
- [ ] **Step 2: Implement** `SecretBackend(Protocol)` (`resolve(self, ref: str) -> str`) and `FileRefBackend`:
  ...
  - `resolve(ref)` → `resolved = confine_to_root(Path(ref), allowed_root=self._root)`; `if not resolved.is_file(): raise PathSafetyError("secret file does not exist")`; `value = resolved.read_text(encoding="utf-8")`; strip a single trailing `\n` (`value[:-1] if value.endswith("\n") else value` — not `rstrip`, which would corrupt a value ending in whitespace); `self._registry.register(value, scope=self._scope)`; `return value`.
