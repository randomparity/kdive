...
`tests/providers/local_libvirt/test_composition.py` (and `tests/providers/test_composition.py` if
it asserts the local set).

Change `frozenset({"offline-vmcore"})` → `frozenset({"offline-vmcore", "live"})`. Update the
inline ADR comment (cite ADR-0219, drop "stays empty until that plane lands"). Test asserts the
descriptor advertises `live`.
...
Confirm `tests/mcp/debug/test_introspect_tools.py` / any tool-docs drift-guard still passes; if a
generated tool-reference snapshot exists, regenerate it.
