...
**Goal:** Reject an `images.upload` whose private project/provider/name is already registered before
any published object write, while preserving concurrent-attempt integrity across architectures.

**Architecture:** Add a registered-name conflict query to the existing PROJECT-locked reservation
transaction in `services/images/upload.py`. Align private pending adoption in
`services/images/publish.py` with registered identity `(owner, provider, name)`, while public
pending adoption remains architecture-scoped. Reacquire PROJECT only in the short private finish
...
- The registered-name check runs under `LockScope.PROJECT` before quota accounting, pending-row
...
- Modify: `tests/services/images/test_publish.py`
...
- Produces: `RegisteredPrivateNameConflict`, a service-specific `CategorizedError` subtype whose
  ...
  RegisteredPrivateNameConflict | None`, called while the PROJECT advisory lock is held. Private
  pending rows of any architecture remain adoptable through `_adopt_or_insert_pending`; public
  pending rows remain architecture-scoped.
...
identity under another architecture. The second reservation must adopt and supersede the first. It
