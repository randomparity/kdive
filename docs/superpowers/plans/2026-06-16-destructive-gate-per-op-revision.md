...
`tests/integration/test_walking_skeleton.py:120-175` — the parametrized gate test has a `(scope_ok, role_ok, opt_in_ok, expected_missing)` row including `(False, True, True, "capability_scope")` (line 127) and seeds `seed_granted_allocation(capability_scope=scope)` (line 141, 172). Rewrite to a two-axis `(role_ok, opt_in_ok, expected_missing)` table; delete the `capability_scope` row; the all-pass row now needs only role + profile opt-in. **Stop passing the `capability_scope=` argument at both call sites (lines 141, 172)** — `seed_granted_allocation`'s param defaults to `None` (`_seed.py:165`), so omitting the argument compiles in this task; do **not** leave a `capability_scope={}` call (Task C removes the param def and does not revisit this file, so a leftover argument would break the Task C commit). Task C then removes only the param definition in `_seed.py`.
