...
`/var/lib/kdive/install(/.*)?`. On a RedHat-family host where `getenforce` reports `Enforcing`,
it installs `policycoreutils-python-utils`, writes both rules with `community.general.sefcontext`,
which converges an existing rule as `semanage fcontext -a` does, and then runs a plain
`restorecon -R` on each directory. `build-image.sh` still owns the nested
