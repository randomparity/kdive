...
formatted`; a local reproduction counted 207. This change's own verification run under
0.16.2 (`uvx ruff@0.16.2 format --check .`) counts **207 files, every one `.md`**, closing
the question: the delta reflected different trees, and the operative figure for this change
is 207.
...
verified by `just ci`. No functional changes mix in; review reduces to a mechanical
commit-shape check whose exact procedure — idempotence under `ruff format --check`, the
per-commit file allowlist, and the docstring-fence content caveat — lives in the spec and
plan under `docs/workflow/` rather than being duplicated here. Formatter output here is not
strictly whitespace: inside Markdown ```python fences it normalizes comment spacing and
blank lines — edits to served-docs content. The test run in `just ci` remains the
behavioral backstop; no Python source file is in the reformat set.
