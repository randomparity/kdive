...
- Follow-ups this decision creates:
  ...
  - Resolve or accept the Grafana ppc64le gap for the opt-in `obs` tier. Interim handling
    landed in #1261: `scripts/live-stack/up.sh` skips grafana on ppc64le and starts prometheus
    on its own, so metrics stay live on POWER (point a workstation grafana at `prometheus:9090`).
    The longer-term posture (mirror a ppc64le grafana à la ADR-0357, replace with a ppc64le-native
    dashboard, or accept the gap indefinitely) remains open under #1261.
