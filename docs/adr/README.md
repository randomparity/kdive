...
| [0160](0160-local-git-build-lane.md) | Local git-clone build lane on the `worker-local` host (agent-supplied repo URL + ref), gated by a deny-by-default operator `KDIVE_LOCAL_BUILD_REMOTE_ALLOWLIST`; enforced at build time on the worker. | Accepted |
