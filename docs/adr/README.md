...
- Move an ADR to **Superseded by NNNN** when a later ADR fully replaces it.
  ...
  form, its date, and that each link — the banner's and the status line's own —
  resolves to a sibling record, in the `## Status` section or in the preamble
  for a pre-0504 record that keeps its status as a bullet there
