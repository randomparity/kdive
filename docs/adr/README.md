...
| [0108](0108-helm-demo-oidc-role-claims.md) | Helm demo OIDC role claims (#369) — the bundled mock-OIDC claim set becomes a `demo.oidc.claims` Helm value defaulting to a full RBAC grant (`admin` on the seeded `demo` project + all three platform roles) so a stock demo deploy is RBAC-testable; `aud:["kdive"]` pinned by the template as a non-overridable invariant; chart-only, inert on the external path | Proposed |
| [0109](0109-reap-leaked-active-allocation.md) | Reap leaked `active` allocations (#371) — the reconciler releases an `active` allocation whose single System is terminal (`torn_down`/`failed`) or absent and that has been settled past a 2-min grace, freeing the host-cap slot without `ops.force_release`; the no-live-System predicate is the complement of admission's non-terminal-System set (so a `crashed` System under live crash-debug is preserved); reuses the shared release mechanic via `reclaim_under_lock` under `PROJECT→ALLOCATION` with an under-lock re-check; wired into `reconcile_once` after the expiry sweep and before promotion (freed slot fills same pass); reported via `reaped_active_allocations` | Proposed |
| [0110](0110-remote-s3-endpoint-guest-routable.md) | Remote install/capture preflight the S3 endpoint as guest-routable (#375) — shared `validate_guest_routable_endpoint()` runs before `presign_get` (install) / `presign_put` (KDUMP capture) and fails fast with `configuration_error` (literal `details.next_action` naming `KDIVE_S3_ENDPOINT_URL`) when the endpoint host is `localhost`/loopback, instead of an opaque downstream in-guest curl failure; no DNS resolution; host-dump (host-side stream) and local-libvirt unaffected; "unset" stays owned by `object_store_from_env` | Proposed |
| [0111](0111-orphaned-domain-name-fallback-reaping.md) | Reap name-orphaned libvirt domains (#372) — the `leaked_domains` reaper resolves a domain's owning System from its `kdive-<uuid>` name when the metadata tag is absent (new `system_id_from_domain_name`), so a genuinely orphaned domain (no DB row, lost tag) is reaped instead of needing out-of-band `virsh undefine`; a name not matching the convention is foreign/unmanaged and never reaped, a live `systems` row (incl. mid-creation `provisioning`) protects it; widens local `list_owned` to surface convention-named untagged domains (`system_id=""`) and wires an always-on libvirt-backed `InfraReaper` into `build_reconciler_reaper` (was `NullReaper` for stock deployments) | Proposed |
| [0112](0112-systems-inventory-config.md) | Declarative systems inventory config (`systems.toml`, #385) — one declarative file owns the operator-configurable inventory (images, provider instances, build hosts, capacity/cost), reconciled into the DB by a merge engine under three ownership layers (config = identity/policy/economics/connection refs; discovery = physical hardware; runtime = realized S3 `object_key`/`digest`/state + project-private rows); a `managed_by` column governs row existence/prune while a config overlay applies declared attributes onto resource `capabilities` (supplies the fault-inject `vcpus`/`memory_mb` #385 lacks); runs as `kdive reconcile-systems` CLI + a reconciler-loop pass (`KDIVE_SYSTEMS_TOML`, ConfigMap in k8s); removes image/host definitions from code (`images/seed_data`, `default_fixtures` YAML, `REMOTE_BASE_IMAGE_NAME`, `KDIVE_REMOTE_LIBVIRT_*` singletons); revises ADR-0092/0093 catalog source + sharpens the ADR-0087 boundary; supports multiple instances per provider; `managed_by` doubles as the declarative(config)/imperative(runtime) boundary so an agent can add/remove a system live (`resources.register`/`deregister`/`renew`, platform_admin) scoped to its project by a new affinity column, with a `lease_expires_at` + reachability reaper collecting leaked runtime additions (cordon+drain, never silent destruction); delivered in 4 phases | Proposed |
| [0113](0113-flat-tool-output-schema.md) | Advertise a flat tool `outputSchema` to stop the recursive-schema client error (#404) — every tool returns the self-referential `ToolResponse` (`items: list[ToolResponse]` + recursive `JsonValue` `data`), so FastMCP auto-derives a recursive `outputSchema` whose `$ref` makes the FastMCP 3.4.0 client's per-call `TypeAdapter` build raise (per-call ERROR log + `CallToolResult.data` returns `None`); `build_app` now sweeps every registered tool after the registrars run and sets `output_schema = {"type": "object"}` (a shared flat constant), removing the recursion for all tools at one chokepoint; the `ToolResponse` model, the `structured_content` wire payload, and the `validate_json_value` runtime check are unchanged (flat schema carries no `x-fastmcp-wrap-result`, so the unwrapped envelope dict is still emitted); chose flat object over `output_schema=None` (keeps the "returns an object" signal), over per-`@app.tool` args (scattered sites, new-tool regression risk), and over a model-level `__get_pydantic_json_schema__` rewrite (would change `ToolResponse.model_json_schema()` for every consumer) | Proposed |
| [0116](0116-granted-set-project-naming.md) | Name authorized projects in the granted-set accounting report (#426) — `accounting.report_granted_set` named no project when it had zero ledger rows (empty `items`, only `total_project:"*"`), so an agent could not learn which project its token grants; the granted-set form now emits one item per *resolved target project*, zero-filling those with no ledger rows in the window (spent projects keep their sums, the `total` row is unchanged); the viewer floor is unchanged (a role-less member still has no accounting read and is dropped from the default set / rejected by the named path — its honest discovery surface is the `projects.list` whoami of #427); zero-fill is granted-set only, leaving `accounting.report_all_projects`'s documented universe-and-`project_count` oversight contract intact | Proposed |
| [0117](0117-projects-list-whoami.md) | Add a read-only `projects.list` (whoami) discovery tool (#427) — no MCP tool reported which projects a caller's token grants, so an agent guessed by trial; `projects.list` projects `current_context()` (grants are token-derived, no DB project table) into a collection of one item per granted project (`{project, role}`, `role:""` for a role-less membership — the discovery gap #426 deferred here) plus top-level `{principal, platform_roles}`, sorted and deduplicated, chaining into `accounting.report_granted_set` via `suggested_next_actions`; a valid token is required (defence in depth via `current_context()`) but no platform/project gate and no audit, since the response is the caller's own token claims with no cross-tenant data; new module + `_PLANE_REGISTRARS` entry + regenerated tool reference, no schema/migration/auth-model change (advertised schema stays the flat `{"type":"object"}`, ADR-0113); chose a per-project collection over a nested-array envelope, an open self-read over a gated one, and a pure projection over a DB-backed registry | Proposed |
| [0118](0118-wait-on-resource-mechanisms.md) | Wait-on-resource mechanisms (#430) — closes three gaps in token-efficient waiting on top of the existing job/lease/queue machinery (`jobs.wait`, `allocations.renew`, FIFO promotion, the `queue_depth` aggregate all already ship): (1) `allocations.wait(allocation_id, timeout_s=30)`, a read-only mirror of `wait_job` that polls until the allocation leaves `requested` (settles to `granted`/`released`/`failed`) or the clamped ≤300s deadline elapses, same authz/no-leak/single-read-on-bad-timeout as `allocations.get`; (2) a per-request `queue_position`/`queue_ahead` hint on a `requested` row (1-based FIFO rank among same-target `requested` rows by `created_at, id`, the order `promote_pending` selects on), surfaced by `get`/`wait` and explicitly advisory not an ETA because promotion is work-conserving/per-host, omitted from `list` to avoid N+1; (3) a `retryable: bool \| None` envelope field derived from `error_category` in the existing `_category_iff_failed` validator (never caller-set, exhaustive-over-enum table with a completeness test, biased terminal when ambiguous so it stops retry-hammering permanent failures), `allocation_denied` classified terminal because it conflates over-budget (terminal) with host-cap-full (whose right recourse is `on_capacity=queue`+`wait`, not blind-retry); one additive nullable column `allocations.failure_category` (the only schema change) lets a failed *queued* allocation report queue_timeout-vs-budget to a waiting agent so the derived `retryable` is correct on the wait path (else a budget-terminated request reads retryable=true and gets re-queued forever); advertised schema stays flat (ADR-0113); chose top-level `retryable` over a `data` key, derived-once over per-call-site storage, position over a historical-timing ETA (deferred), and per-object `allocations.wait` over a combined `resources.wait` | Proposed |
| [0119](0119-operator-build-config-write-path.md) | Operator write-path for build-config fragments (#438) — adds the `platform_admin`-gated, `platform_audit_log`-audited `buildconfig.set` MCP tool that publishes/replaces a kernel-config fragment to the existing reserved object key + upserts `build_config_catalog`, so an operator changes a kdump (or any) policy without rebuilding the image; system-scoped `platform_admin` (not project `admin`) because the catalog has no project, mirroring break-glass `ops.force_*`; a new `source` provenance column (`seed`|`operator`, migration `0034`) lets the deploy seed refresh seed-owned rows while never clobbering an operator override on a later `migrate`; name charset-validated and content size-capped (`KDIVE_MAX_BUILD_CONFIG_BYTES`) at the write boundary; `mutating` not `destructive`; `buildconfig.get`, the build-path fetch, the reserved-key scheme, and the flat schema unchanged | Proposed |
