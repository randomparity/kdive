...
| [0159](0159-guest-agent-deterministic-failure-classification.md) | Subcategorize a deterministic guest-agent libvirt error (agent not configured / permission denied / unsupported) as `configuration_error` (`retryable=false`) instead of `transport_failure`; keep transport for transient/unknown drops; surface the libvirt error string + code in `details`. | Proposed |
