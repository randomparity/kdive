...
| [0163](0163-diagnostics-worker-vantage-dispatch.md) | Wire worker-job dispatch so `ops.diagnostics`' `provider_tls`/`gdbstub_acl` run for real on the worker (bounded-wait, single coherent verdict); `WORKER_UNAVAILABLE` on worker-down. | Accepted |
