...
| [0158](0158-runs-cancel-tool.md) | `runs.cancel` drives a non-terminal Run to terminal `canceled` under the per-Run lock (idempotent on already-`canceled`, `conflict` on `succeeded`/`failed`), best-effort cancels the build job, frees the System without teardown. | Proposed |
