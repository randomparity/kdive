...
When a matching tombstone is accompanied only by canonical `intent.json` or
`preparation-result.json`, the store validates every present record against the tombstone's binding,
plan, materialization, recovery reference, and source/target identities. It then removes only those
two literal filenames and fsyncs the directory before reporting cleanup accounted. Every retry
repeats the same comparisons. A malformed record, symlink, non-private inode, mismatched identity,
or any other directory entry fails closed without removing anything.
...
- A crash during tombstone publication is resumable at each file-removal boundary.
