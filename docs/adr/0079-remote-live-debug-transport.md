...
- **gdb-MI: direct TCP from the worker to the host's gdbstub port**, over a configured,
  operator-firewalled port range, under an **explicit, documented network assumption** (routable
  worker→host TCP on those ports). The provisioning profile (spec issue 2) defines the domain
  with the gdbstub enabled; the Connect port hands the worker's gdb a `host:port`. Bare metal
  later swaps the gdbstub for KGDB-over-SoL **behind the same Connect port**.
- **drgn live introspection: in-guest, via the qemu-guest-agent seam** (ADR-0078) — the worker
  drives a constrained, allowlisted drgn invocation inside the guest and streams results back;
  no SSH login and no new channel.
...
- **Documented operator responsibility.** Worker→host reachability on the gdbstub port range
  (and the libvirtd TLS port, ADR-0077) is a network/firewall precondition the operator owns;
  it is recorded in the spec and the e2e runbook, not assumed silently.
...
- **Unreachable gdbstub / guest agent maps to `transport_failure`; a reference invalidated by a
  reboot or reprovision maps to `stale_handle`** — both existing categories; no new strings.
- **Cost / limitation: direct TCP assumes a routable path.** In a segmented network this needs
  an operator-provided route or jump. A TLS-tunneled gdbstub proxy is a possible later
  hardening if a deployment cannot route directly; M2 does not build one (it would add a
  deployed proxy + cert material not obviously reused by cloud/bare-metal).
