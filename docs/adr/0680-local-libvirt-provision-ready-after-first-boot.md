...
1. **Wait in the provider.** After define/start, the job-lane `LocalLibvirtProvisioning.provision`
   ...
   answers. The poll count is `ceil(KDIVE_LIBVIRT_BOOT_WINDOW_S / 5)` scaled by
   `tcg_deadline_multiplier(accel)` (ADR-0341), where `accel` is the one provision already
   resolves from live capabilities. `reprovision` inherits the wait. The probe is an injected
   seam; a provisioner built without it (unit tests, the authority lane) does not wait.
...
3. **Failure.** The window elapsing, a crash signature before the marker, or the domain exiting
   ...
   the existing closed `probe_error` and `crash_signature` values. The handler already drives the
   System `failed` with that category and dead-letters the job.
...
5. **Retry.** When the defined domain is already active, provision neither truncates the console
   nor calls `create()`; it waits on the existing log. That domain was started by an earlier
   attempt of the same provision after its own truncate, so the log holds the whole boot.
6. **Authority lane unchanged.** Its provisioner gets no wait seam, so the marker is waited for
   once, by the lane's existing `boot_ready` probe. It does benefit from decision 5.

## Consequences

- A local-libvirt provision or reprovision job now lasts through the guest's first boot: about a
  minute or two on KVM, longer under TCG, bounded by the scaled boot window. The worker heartbeat
  renews the job lease for the whole wait (ADR-0018). Minting paths such as
