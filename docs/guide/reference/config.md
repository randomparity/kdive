...
| `KDIVE_MEASUREMENT_TIMEOUT_S` | `1800` | Client request timeout the #2318 finalization-measurement driver runs under, so a large bundle completes and can be measured at all. Distinct from the 30-second supported client budget the resulting decision is tested against (ADR-0655), which is a module constant rather than a knob so the environment running the proof cannot move the threshold the accepted decision rests on. |
