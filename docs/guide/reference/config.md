...
| `KDIVE_UPLOAD_ORPHAN_GRACE_SECONDS` | reconciler | `86400` | no | Grace window in seconds protecting an unreferenced object under an upload prefix from the reconciler's orphan sweep (ADR-0455). Measured from the object's store mtime and applied on top of KDIVE_UPLOAD_TTL_SECONDS, so an object is reclaimed only well after the window it could have belonged to was reaped. Raise it to stall the sweep. |
