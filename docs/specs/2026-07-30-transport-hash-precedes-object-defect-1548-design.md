...
| A store fault during the drain (ADR-0523 Consequences) | `test_strip_gzip_store_fault_during_the_drain_keeps_the_decode_diagnosis` — the store's own error passes through unreclassified with its own `__cause__` intact, and the decode diagnosis it would otherwise have destroyed arrives as a `__notes__` entry. |
