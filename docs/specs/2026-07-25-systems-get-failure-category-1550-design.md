...
4. **Both job-backed writers force `exc.terminal = True`** immediately after recording the System
   ...
   `failure_context` and leaves `error_category` NULL), so a `failed` row is exactly the row that
   carries an answer. Filtering the lookup *to* `failed`, however, would skip over a newer
   terminal job to reach a stale older failure — see the query below.
6. **A dead-lettered job does not always carry a message.** `repair_abandoned_jobs` stamps
   `lease_expired` on any zombie job while leaving `failure_context` untouched, and
   `repair_stalled_restoring_systems` runs after it, so "a job with no reason" is a normal shape,
   not an edge case.
7. **`not_found` / `authorization_denied` are reserved envelope-level meanings** (ADR-0097).
   `get_system` itself returns `not_found` for an absent or invisible System.
...
`kind = ANY(SYSTEM_FAILING_JOB_KINDS)` AND `payload->>'system_id' = %s` AND
`state = ANY(succeeded, failed, canceled)`, newest first by `(created_at, id)`, `LIMIT 1` —
returning the row **only if** its state is `failed`.

Deliberately not `state = 'failed'` with a `LIMIT 1`: that skips *over* a newer terminal job to
reach a stale older failure. Cancelling a `restore` satisfies `repair_stalled_restoring_systems`'
"no restore job queued/running" predicate, so the System reaches `failed` with nothing to
attribute, and an older failed `provision` row would then be reported as authoritative — worse
than the default it replaces, because it looks certain.
...
A no-leak category (ADR-0123) is dropped **whole**, not half-quoted: the category degrades to
`infrastructure_failure`, `detail` becomes a distinct "this surface cannot report it; read the
System's jobs" string (not the "no job recorded a reason" one, which would be false), and
`failing_job_id` is withheld. Gated on `suppressed_detail(category, None) is not None`.

Every derived reason is additionally gated on a `failure_attributed` flag that `get_system` sets
and `list_systems` does not. A bare `failing_job=None` cannot distinguish "looked and found none"
from "never looked", and the list path never looks — so it keeps its pre-existing `detail=None`
silence rather than asserting a fact nobody checked.
...
4. FAILED + a job on a no-leak category (both of them) → category degrades to
   `infrastructure_failure`, detail is the unreportable-reason string, no `failing_job_id`,
   `jobs.list` leads the next actions, and the job's message does not appear anywhere.
...
8. The failure envelope names the recovery actions, and every one is a registered tool.
9. FAILED with `failure_attributed` unset (the list path's shape) → `detail is None`; no derived
   reason is asserted.

Integration, over the helper and `get_system` (DB):

6. `latest_failed_job_for_system` returns the newest failed provision job for the System, and
    returns `None` when a newer `canceled` or `succeeded` job of a matching kind exists.
...
11. `list_systems` on the same FAILED System still reports `infrastructure_failure` **and
    `detail is None`** — pinning both the disclosed scope and the rule that the list path never
    claims a reason does not exist.
