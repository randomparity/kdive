...
4. Return `True` (defer the drain) when a rootfs row survives, when a live writer held a partial, or
   when a protected token's file was left behind.
...
`sweep_unowned_investigation_rootfs_staging(conn, retention)` — a third lane whose worklist is
...
- A base a live System pins is left in place and the drain marker retained.
