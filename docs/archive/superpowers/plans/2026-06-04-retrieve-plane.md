...
- [ ] **Step 3:** Create `src/kdive/mcp/tools/vmcore.py`. Structure (mirror `control.py` + `runs.py`):
  ...
  - `capture_handler(conn, job, retriever)` — three phases, mirroring `build_handler` (capture runs with **no DB transaction held**). `system_id = UUID(job.payload["system_id"])`.
    1. **Pre-check (short txn + per-System lock):** `async with conn.transaction(), advisory_xact_lock(conn, LockScope.SYSTEM, system_id):` load the System (None → raise `CategorizedError(INFRASTRUCTURE_FAILURE)`); query an existing raw `vmcore` row — `SELECT object_key FROM artifacts WHERE owner_kind='systems' AND owner_id=%s AND object_key LIKE %s` with the param `'%/vmcore'` (this does **not** match `…/vmcore-redacted`). If a row exists, return its `object_key` (idempotent re-dispatch — no re-capture). The lock is released when the `async with` exits.
    2. **Capture (no txn):** `out = await asyncio.to_thread(retriever.capture, system_id)`. A `CategorizedError` (e.g. `READINESS_FAILURE` for no core, `INFRASTRUCTURE_FAILURE` for a store fault) propagates out of the handler so the worker dead-letters the job with that category — do not catch it.
    3. **Finalize (short txn + per-System lock):** re-open `conn.transaction()` + the lock; re-run the existing-row check (a concurrent re-dispatch may have won — if a row now exists, return its key without inserting); else `ARTIFACTS.insert(conn, register_artifact_row(out.raw, owner_kind="systems", owner_id=system_id))` and the same for `out.redacted`, then `audit.record(... tool="vmcore.fetch", object_kind="systems", object_id=system_id, transition="capture_vmcore", ...)`, and return `out.raw.key` as the `result_ref`.
