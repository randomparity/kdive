...
**Rollout safety (prune never deletes S3 bytes — corrected back to ADR-0112):** the first post-migration reconcile prunes any config image absent from `systems.toml`. Prune is **row-delete-only**: it deletes the catalog *row* and **never** calls `store.delete` on the object. So an **idle baseline the operator forgot to list** loses only its row — the bytes are untouched and are reclaimed by the existing `repair_leaked_images` reconciler sweep once the object is rowless past the publish grace (that sweep is **not** visibility-scoped and enumerates `Prefix="images/"`, so it covers public objects). No grace-TTL tombstone and no report-only first pass are needed, because nothing irreversible happens on any pass. The cordon guard (Task 1.5) still protects an in-use image's row from deletion, so its bytes are never orphaned while a live System references it. *(This note previously specified an inline-reclaim + grace-TTL tombstone valve; that drifted from this plan's own ADR-0112 — which mandates row-delete-only — on the false premise that no public-image GC exists. `repair_leaked_images` (`reconciler/images.py`, wired live at `loop.py`) is exactly that GC. Corrected to match the ADR; #390 implements row-delete-only.)*
...
- Prune: for each existing `managed_by='config'` row whose `(provider,name,arch)` is absent from config, call `_prune_or_cordon`.
  ...
  - If referenced → **cordon** + `diff.cordoned` (no delete). If idle → **delete the row only** (`DELETE FROM image_catalog`); **never `store.delete` inline** (ADR-0112). The S3 object is reclaimed by the existing `repair_leaked_images` sweep once it is rowless past the publish grace; a `staged` row (`object_key IS NULL`) has no object at all. This makes prune non-destructive of bytes: a file edit can never irreversibly delete image data.
  - **Why row-delete-only, not inline reclaim (corrected — reconciles to ADR-0112):** ADR-0112 mandates "Pruning an image never deletes its S3 object inline; object reclamation stays the existing image-GC path." That GC is `repair_leaked_images` (`reconciler/images.py`): it deletes any `images/`-prefix object with **no catalog row**, is **not** visibility-scoped (it covers public objects), and is wired live into the loop (`loop.py` `_RepairSpec("leaked_images", …)`). So a pruned public row's object is collected once rowless past the grace — it does **not** leak forever. *(An earlier draft of this task specified inline `store.delete` + a per-image object-before-row transaction + a grace-TTL tombstone, justified by "there is no public-image GC." That premise was false; `repair_leaked_images` is that GC. #390 implements row-delete-only.)*
  - Runtime/discovery rows: skip (invariant 2/3).
- **Transaction boundaries:** batch the **upserts** in one transaction (all-or-nothing for the create/update phase). Run each **prune in its own transaction** (the liveness re-check `FOR UPDATE` and the row delete are atomic per row, so one row's outcome never half-commits another's). Because prune is row-delete-only there is no S3 side effect to coordinate — a crash leaves at most an uncommitted delete the next pass re-prunes idempotently. The whole pass holds one **session-scoped** inventory advisory lock across these transactions.
...
- [ ] **Step 4:** Until a kind is demonstrably covered by the guard, prune of that kind stays **cordon-only** in Task 1.4 (never deletes the row) — assert that fallback in a test.
- [ ] **Step 5: Commit**
