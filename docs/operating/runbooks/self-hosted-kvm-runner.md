# Runbook: self-hosted Rocky 10 KVM runner
...
A Rocky Linux 10 host that satisfies the sub-issue A `live_vm` environment
contract (`tests/live_vm/__init__.py`): `/dev/kvm` + native qemu, the modular
libvirt daemons, the `default` pool/network, the kernel-debug toolchain (`drgn`,
`crash`, `makedumpfile`, `kexec-tools`, `kdump-utils`, `gdb`, foreign qemu), a
`virt_image_t`-labeled world-traversable staging tree, a short `XDG_RUNTIME_DIR`,
and a registered — but **not yet listening** — GitHub Actions runner service.

## Roles and playbook

`deploy/ansible/playbooks/runner.yml` applies, in order:

1. `libvirt_stack` (reused) — qemu/libvirt/libguestfs, RHEL modular daemons, KVM assertion.
...
3. `live_vm_host` — the contract delta: service-account groups, the toolchain,
   ...
   `virt_image_t`, `enable-linger`, and the two-part host-contract gate.
4. `github_runner` — the runner asset (checksum-verified), registration, and the
   systemd service (installed stopped).

## Prerequisites

- Rocky Linux 10 host (x86_64; ppc64le is a drop-in — see below), SSH-reachable
  as a `become`-capable account.
...
- The host entry: add the host under the `live_vm_runners` group in
  ...
  `host_vars/rock10-runner.yml`) with its `ansible_host` and
...
(default `/opt/kdive`), built against the **system** interpreter
(`uv sync --python /usr/bin/python3 --group live`) so the symlinked `libguestfs`
native module ABI-matches. The `live_vm` CI job (sub-issue D) **must** reuse this
...
- **After a kernel upgrade**, re-run `playbooks/runner.yml` (or re-apply
  `chmod 0644 /boot/vmlinu?-*`): a new `/boot/vmlinuz-*` ships `0600 root:root`,
...
- **Host contract:** `live_vm_host`'s gate runs `scripts/check-local-libvirt.sh`
  as the runner user and asserts the `virt_image_t` label, `/boot` readability,
  group membership, and `/run/user/<uid>`; the play fails if the host is not
  ready.
