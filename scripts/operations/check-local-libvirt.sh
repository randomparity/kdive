...
# BASH_SOURCE[0] is often relative (`bash scripts/operations/check-local-libvirt.sh` from the repo root gives a
# single-slash path), so anchor it to $PWD first (a builtin, unlike dirname) to stay CWD-independent;
# without this the strips below miss the repo venv and silently fall back to system python3.
# `${var%/*}` strips one component; three applications (filename, operations/, scripts/) give the
# repo root, then append `.venv/bin/python`.
  ...
  for dir in "${ROOTFS_DIR}" "${INSTALL_STAGING}"; do
    ...
    if [[ "${label}" == svirt_image_t ]]; then
      ...
      note_fail "${dir} is labeled ${label}, not svirt_image_t (SELinux enforcing; a confined domain cannot write or map its images, ADR-0640)" \
        "run 'KDIVE_LIFECYCLE_WITNESS_DATABASE_URL=... just prepare-local-libvirt-host', which installs the svirt_image_t fcontext rules and restorecons ${dir}"
    fi
  done
fi

# Warn when the effective libvirt identity is non-root under qemu:///system: that identity
# cannot read back the root-owned console log / core virtlogd/QEMU write.
if [[ "${LIBVIRT_URI}" == "qemu:///system" && "${EFFECTIVE_UID}" -ne 0 ]]; then
  note_warn \
    ...
    "set KDIVE_LIBVIRT_URI=qemu:///session (worker-owned QEMU), or grant the worker group read access to the libvirt/virtlogd output; build and kdump capture still work as-is"
fi
