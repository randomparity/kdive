...
# permanently (ADR-0636). Overridable so the probe is testable without an EL host.
...
# because on EL the arch-named binary does not exist (ADR-0636 decision 3).
...
package_for() {
  ...
  # The RedHat family answers by NATIVENESS, not by architecture (ADR-0636). `qemu-kvm` is a
...
probe_all() {
  ...
  # PATH-only, and the RedHat family keeps this host's own emulator off PATH (ADR-0636 decision 2).
