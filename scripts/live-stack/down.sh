...
# kdive-minio-data, kdive-prom-data, kdive-build, kdive-install (ADR-0552) -- AND reaps
...
if [[ "$wipe" == "1" && "$assume_yes" != "1" ]]; then
  echo "WARNING: --wipe drops the compose data volumes (database, artifacts bucket, metrics" >&2
  echo "TSDB, build/install caches) and destroys all kdive-* libvirt domains" >&2
