...
async def test_barrier_is_closed_until_one_exact_arm_reaches_and_releases(tmp_path: Path) -> None:
    ...
    try:
        ...
        unmatched = asyncio.create_task(
            barrier.checkpoint(uuid4(), AuthorityOperation.ACTIVATE, "before-provider")
        )
        await unmatched
        checkpoint = asyncio.create_task(
            barrier.checkpoint(system_id, AuthorityOperation.ACTIVATE, "before-provider")
        )
        for _ in range(20):
...
async def test_shutdown_aborts_a_reached_checkpoint_without_releasing_it(tmp_path: Path) -> None:
    ...
    system_id = uuid4()
    try:
        await barrier.arm(system_id, AuthorityOperation.ACTIVATE, "after-provider")
        checkpoint = asyncio.create_task(
            barrier.checkpoint(system_id, AuthorityOperation.ACTIVATE, "after-provider")
        )
