...
import kdive.processes.lifecycle.systemd.systemd_worker_state as worker_state
...
def test_inspect_reports_each_slot_residue(
    store: SlotStore, settings, started_state: SlotState
) -> None:
    """Cases 1 and 2 of #2533 are invisible while `load` collapses them into None and a raise."""
    assert store.inspect() == worker_state.SlotInspection(worker_state.SlotResidue.EMPTY, None)

    prepared = store.prepare(settings)
    assert store.inspect() == worker_state.SlotInspection(
        worker_state.SlotResidue.STATE_VALID, prepared
    )

    store.state_path.write_bytes(b"{ not json")
    assert store.inspect() == worker_state.SlotInspection(
        worker_state.SlotResidue.STATE_UNREADABLE, None
    )

    store.state_path.unlink()
    assert store.inspect() == worker_state.SlotInspection(
        worker_state.SlotResidue.STATE_ABSENT, None
    )


def test_inspect_returns_unreadable_instead_of_raising_for_a_malformed_document(
    store: SlotStore, settings
) -> None:
    """`load` raises StateConflict here, which is what wedges the whole recovery sweep."""
    store.prepare(settings)
    store.state_path.write_bytes(b'{"schema": 1, "slot":')

    with pytest.raises(StateConflict):
        store.load()

    assert store.inspect().residue is worker_state.SlotResidue.STATE_UNREADABLE
    assert store.inspect().state is None


def test_discard_unrecoverable_clears_a_slot_with_no_parseable_state(
    store: SlotStore, settings
) -> None:
    store.prepare(settings)
    store.state_path.write_bytes(b"{ not json")
    ...
    assert not store.credential_path.exists()
    assert not store.release_path.exists()
    assert store.slot_path.is_dir()


def test_discard_unrecoverable_reports_whether_it_removed_anything(
    store: SlotStore, settings
) -> None:
    """The installer pre-creates every slot directory, so the return is the only honest signal."""
    store.prepare(settings)
    assert store.discard_unrecoverable() is True
...
def test_discard_unrecoverable_requires_root(
    ...
    store.prepare(settings)
    monkeypatch.setattr(worker_state.os, "geteuid", lambda: 1000)
...
def test_discard_unrecoverable_accepts_an_already_empty_slot(store: SlotStore) -> None:
    assert store.discard_unrecoverable() is False
