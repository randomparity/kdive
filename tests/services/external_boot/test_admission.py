...
from tests.services.external_boot.admission_support import GUARDED_TOOLS, UNGUARDED_TOOLS
...
def test_every_registered_mutating_tool_is_guarded_or_exempt() -> None:
    ...
    """
    decided = GUARDED_TOOLS.keys() | UNGUARDED_TOOLS.keys()
    mutating = {
        name
    ...
    assert GUARDED_TOOLS.keys().isdisjoint(UNGUARDED_TOOLS)
    ...
    assert all(reason.strip() for reason in UNGUARDED_TOOLS.values())
...
def test_every_operation_the_matrix_decides_has_a_guarded_tool() -> None:
    ...
    assert set(GUARDED_TOOLS.values()) == set(ExternalBootOperation)
