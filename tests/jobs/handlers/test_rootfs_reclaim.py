...
@contextmanager
def _held_lease(uploads: Path, inv: UUID, token: str = _TOKEN) -> Iterator[Path]:
    """A ``<token>.<uuid>.fetching`` under an exclusive ``flock``, as a live fetcher holds it.

    Deliberately creates **no** partial. That is the whole point of ADR-0515's window: the fetcher
    has resolved its ``artifacts`` row and is waiting on the per-(investigation, checksum) session
    lock, so it has not opened a partial yet and the ADR-0495 probe alone finds nothing to defer on.
    """
    inv_dir = uploads / str(inv)
    inv_dir.mkdir(parents=True, exist_ok=True)
    lease = staged_rootfs_fetch_lease_path(inv_dir / f"{token}.qcow2", uuid4().hex)
    lease.touch()
...
def test_a_held_fetch_lease_pins_the_base_with_no_partial_yet(
    ...
    # the artifacts row is resolved, the partial does NOT exist yet, and the System that requested
    # the base is in a pin-dropping terminal state. Neither existing gate sees the download —
    ...
    # reclaimable, and there is no partial for the ADR-0495 probe to find. Before ADR-0515 the base,
    # the object and the row were all deleted out from under a live download. Both terminal states
    # are parametrized because the issue names both and they fail the classifier for *different*
    # reasons; a fix that closed only one would pass a single-state test.
    inv = uuid4()
    ...
    async def _run() -> None:
        ...
        try:
            inv = await _seed_investigation(seed, state="closed", closed=True)
            artifact_id = await _seed_rootfs_row(seed, inv)
            await _seed_system(seed, inv, system_state, _upload_profile())
        finally:
            await seed.close()
        rootfs_dir, uploads = _dirs(tmp_path)
        staged = _stage(uploads, inv)
        ...
        with _held_lease(uploads, inv) as lease:
            assert not list((uploads / str(inv)).glob("*.partial")), "the window has no partial"
            result = await _run_handler(
                migrated_url, inv, [artifact_id], store, rootfs_dir, uploads
            )
            assert result == "0"  # deferred, not drained — and the job still succeeded
            assert lease.exists(), "the gate is read-only and must not take the live lease"
            assert staged.exists(), "the base was unlinked under a live download"
            assert staged_rootfs_marker_path(staged).exists()
        ...
        # The fetcher is gone, so the very next pass drains with no manual repair. This is the
        # bounded half of the pin and the reason the evidence is a flock rather than a column: the
        # kernel drops it when the holding descriptor closes, including on SIGKILL.
        assert await _run_handler(migrated_url, inv, [artifact_id], store, rootfs_dir, uploads) == (
            "1"
        )
        assert not staged.exists()
        assert store.deleted == [_object_key(inv)]
        assert not (uploads / str(inv)).exists()  # the now-orphan lease went with the drain
        check = await connect(migrated_url)
        try:
            assert not await _row_exists(check, artifact_id)
            assert await _marker(check, inv) is None
...
def test_a_crash_orphan_fetch_lease_does_not_pin_the_base_forever(
    migrated_url: str, tmp_path: Path
) -> None:
    # AC-8's property, in the form ADR-0515 gives it. The regression AC-8 guards is a pin nothing
    # can ever release: a base pinned by a dead fetcher is a SENSITIVE multi-GiB leak of up to the
    # 50 GiB canonical cap, and it is exactly what a durable *column* set by a SIGKILLed fetcher
    # would produce. So the lease pins only while its holder does. Here the file is present and its
    # fetcher is gone, and the checksum drains in the very same pass that would have deferred on a
    # held one — base, object and row all reclaimed, and the orphan lease collected by the drain
    # tail rather than left to keep the directory non-empty forever.
    inv = uuid4()
    ...
    async def _run() -> None:
        ...
        try:
            inv = await _seed_investigation(seed, state="closed", closed=True)
            artifact_id = await _seed_rootfs_row(seed, inv)
            await _seed_system(seed, inv, "failed", _upload_profile())
        finally:
            await seed.close()
        rootfs_dir, uploads = _dirs(tmp_path)
        staged = _stage(uploads, inv)
        orphan = staged_rootfs_fetch_lease_path(staged, "deadbeef")
        ...
        assert not orphan.exists(), "an unheld lease was left to keep the staging dir alive"
        assert not staged.exists()
        assert store.deleted == [_object_key(inv)]
        assert not (uploads / str(inv)).exists()
        check = await connect(migrated_url)
...
def test_a_live_lease_of_another_token_does_not_defer_this_checksum(
    migrated_url: str, tmp_path: Path
) -> None:
    # The lease probe inherits the partial probe's scoping, and must: the staging tree is per
    # investigation, so a sibling System fetching a *different* base is a routine concurrent state.
    # Deferring this checksum on that lease would stall it for the length of an unrelated multi-GiB
    # download — ADR-0442 §7's starvation, keyed on an unrelated name.
    inv = uuid4()
    other_token = rootfs_object_token(_CHECKSUM_Y)
    ...
    async def _run() -> None:
        ...
        with _held_lease(uploads, inv, other_token) as live:
            assert await _run_handler(
                migrated_url, inv, [artifact_id], store, rootfs_dir, uploads
            ) == ("1")
            assert not staged.exists()
            assert store.deleted == [_object_key(inv)]
            assert live.exists(), "the sibling's live lease was destroyed"
