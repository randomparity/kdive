...
def test_whoami_empty_context_keeps_every_key_present() -> None:
    ctx = RequestContext(principal="agent-bare", agent_session=None, projects=())
...
def test_whoami_does_not_leak_agent_session() -> None:
    ctx = RequestContext(
        principal="agent-1", agent_session="secret-correlator", projects=("proj-a",)
    )
