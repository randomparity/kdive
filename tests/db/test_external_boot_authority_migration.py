...
def test_teardown_allocation_admits_supported_nonterminal_system_state(
    ...
    with psycopg.connect(migrated_url) as conn:
        assert conn.execute(
            "SELECT a.state, c.last_generation, s.state "
            "FROM external_boot_authorities AS a "
            "JOIN external_boot_authority_counters AS c ON c.system_id=a.system_id "
            "JOIN systems AS s ON s.id=a.system_id WHERE a.id=%s",
            (authority.authority_id,),
        ).fetchone() == ("allocating", authority.generation, system_state)
        assert conn.execute(
