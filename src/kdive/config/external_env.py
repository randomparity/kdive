...
EXTERNAL_ENV_VARS: tuple[ExternalEnvVar, ...] = (
    ...
    ExternalEnvVar(
        ...
        "per-run container (ADR-0401); unset → one shared testcontainer is started per run. Each "
    ...
    ExternalEnvVar(
        ...
        "test",
        None,
        "Presence gate for the isolated executable Compose worker-lifecycle proof; the dedicated "
    ...
    ExternalEnvVar(
        "KDIVE_WORKER_INCARNATION_NONCE",
        "script",
        None,
        "Ephemeral 128-bit nonce generated by the reference Compose lifecycle gate and passed "
    ...
    ExternalEnvVar(
        ...
        "Enterprise Linux host as permanently broken (ADR-0636). Native arch only — this binary "
    ...
    ExternalEnvVar(
        ...
        "script",
        "1",
        "Whether the local Compose reference provisions its fixed development-only runtime login "
