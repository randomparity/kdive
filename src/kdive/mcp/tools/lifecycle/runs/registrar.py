...
class _RunsCreatePayload(ToolPayload):
    ...
    build_profile: ExternalBuildProfile | ServerBuildProfile = Field(
        description=(
            ...
            "For a source='server' build, the optional 'config' is a catalog ComponentRef, or a "
            "list of them to compose: paste the config_ref that buildconfig.set/list/get echo "
            "(e.g. {'kind':'catalog','provider':'system','name':'kdump'}), which fills in the "
            "required provider for you. A list merges its fragments in order (a later fragment "
            "wins on a conflicting symbol) and fully replaces the default - list the kdump "
            "config_ref explicitly to keep it. Omit it to get the seeded kdump fragment (KEXEC, "
            ...
            "options and you only add on top. Whatever config you supply, the built .config must "
            "still satisfy buildconfig.get data.platform_required_config (rootfs-mount plus "
            "crash-dump/debuginfo symbols) or the build fails with a configuration_error. Use "
            "runs.validate_profile to pre-flight a profile without creating a Run. Extra "
