...
async def build_handler(conn: AsyncConnection, job: Job, builder: Builder) -> str | None:
    ...
    result = await plane._existing_build_result(conn, run_id)
    if result is None:
        try:
    ...
    await plane._finalize_build(conn, job, run, result)
    return str(run_id)
...
async def install_handler(conn: AsyncConnection, job: Job, installer: Installer) -> str | None:
    ...
    method = plane._install_method_for(system)
    kernel_ref = run.kernel_ref
    cmdline = await plane._cmdline_for(conn, run, method)
    _log.info("install: run %s resolved cmdline %r (method %s)", run_id, cmdline, method.value)
    initrd_ref = await plane._installed_initrd_ref(conn, run_id)
    ...
    try:
        ...
        try:
            raw = await asyncio.to_thread(
                plane.read_console_log, plane.console_log_path(run.system_id)
            )
            if not raw:
                _log.warning(
                ...
                stored = await asyncio.to_thread(
                    lambda: plane.object_store_from_env().put_artifact(
                        "local",
                        "systems",
                        str(run.system_id),
                        "console",
