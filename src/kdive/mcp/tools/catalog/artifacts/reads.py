...
# The largest object pulled whole into memory for an `artifacts.get` window or `find` search
# (ADR-0247, ADR-0283). Larger objects omit inline content from a plain read and reject a `find`
# (`artifact_too_large`); the always-present `refs.download_uri` serves them.
...
_FIND_DESCRIPTION = (
    "Optional literal search terms; omit for a plain windowed read. '|' separates alternatives "
    "(e.g. 'BUG: KASAN|Call Trace'); the nearest term in direction is returned with "
    "data.match_offset, data.match_line, the surrounding data.content, and data.next_offset to "
    "continue (data.match_found is false when none remain). Per-line literal substring, "
    "case-sensitive, no regex and no Unicode normalization — match the artifact's exact bytes "
    "(kernel signatures are ASCII). An artifact above the fetch ceiling cannot be searched and "
    "is rejected with configuration_error data.reason=artifact_too_large, never a silent "
    "match_found=false."
)
_BYTE_OFFSET_DESCRIPTION = (
    "Start byte of the window, or of the search cursor when find is set (0-based). With "
    "direction=forward (default) a negative value starts from the beginning; with "
    "direction=backward the window (or search) runs up from this byte and an omitted (0) or "
    "negative value means end-of-artifact. Page with the returned data.next_offset until "
    "data.content_truncated is false, or — with find — until data.match_found is false."
)
_MAX_BYTES_DESCRIPTION = (
    "Maximum inline window bytes, or surrounding content bytes on a find match; default "
    f"{ARTIFACT_GET_WINDOW_DEFAULT_BYTES}, sized to the tool-result token budget. The server "
    "caps the window at the smaller of a hard "
...
_GET_DIRECTION_DESCRIPTION = (
    "Cursor direction for paging and for find. forward starts at byte_offset (the artifact "
    "start when omitted). backward starts at end-of-artifact when byte_offset is omitted (read "
    "the tail and page up); a positive byte_offset bounds it."
)


class ArtifactsGetRequest(ToolPayload):
    """Request payload for ``artifacts.get`` byte-window reads and literal jump searches."""

    artifact_id: str = Field(description=_ARTIFACT_ID_DESCRIPTION)
    find: str | None = Field(default=None, description=_FIND_DESCRIPTION)
...
async def artifacts_get(
    ...
    *,
    ...
    """Return one `redacted` artifact's content window, or jump to a literal match in it.

    With ``find`` absent this returns a byte window:
    ``data[byte_offset : byte_offset + effective_max]`` where
    ...
    With ``find`` set the call jumps to the nearest literal ``|``-OR match in ``direction`` over
    the whole body (#939, ADR-0283): ``data["match_found"]`` plus, on a hit,
    ``data["match_offset"]``, ``data["match_line"]``, the surrounding ``data["content"]`` window,
    and a direction-relative ``data["next_offset"]`` to continue. Matching is byte-space literal
    (no regex, no Unicode normalization).
    ...
    ``refs["download_uri"]``; a ``find`` over one cannot search bytes that were never fetched, so
    it rejects with ``configuration_error`` ``reason=artifact_too_large`` rather than a misleading
    ``match_found=false``. A store outage degrades to a ``data["content_unavailable"]`` reason;
    ...
    if request.find is not None:
        try:
            terms = parse_literal_terms(request.find)
