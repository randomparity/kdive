...
LIBVIRT_RECOVERY_ROOT = Setting(
    ...
    help=(
        ...
        "running worker account — which the recovery stores re-check on every open. Two "
        "conditions the stores cannot check for themselves and provisioning must supply: "
        "every ancestor directory of this path must be owned by root or by the worker "
        "account and must not be group- or world-writable, because the per-open checks "
        "constrain only the final component; and the worker process umask must not clear an "
        "owner bit (0700 and 0077 are fine, 0177 is not), because a masked mkdir yields a "
        "directory that fails the mode-0700 check it was just created to satisfy. This is "
    ...
    suggest=(
        ...
        "account running this process, with every ancestor owned by root or that account "
        "and not group- or world-writable, and with the worker unit's UMask leaving the "
        "owner bits alone (UMask=0077); provisioning creates one per slot under "
