"""Bounded local-libvirt authority mutation adapter (ADR-0584)."""
...
type OpenConnection = Callable[[str], _Connection]


class LocalLibvirtAuthorityMutationAdapter:
    """Observe and mutate one System through one configured local libvirt URI."""

    def __init__(self, uri: str, *, open_connection: OpenConnection = libvirt.open) -> None:
        self._uri = uri
    ...
    async def commit(
        ...
        connection = self._open_connection(self._uri)
        try:
            ...
            raise CategorizedError(
                "local-libvirt authority mutation failed",
...
__all__ = ["LocalLibvirtAuthorityMutationAdapter", "SUPPORTED_COMMIT_POINTS"]
