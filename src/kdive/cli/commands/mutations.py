...
These tools are ``destructive()``-annotated server-side. Their descriptor-owned command path is
paired with ergonomic specialised execution. The generic ``tool call`` passthrough can also reach
a destructive tool, but only with an explicit ``--allow-destructive`` opt-in plus confirmation
(ADR-0107), so neither path is silent.
